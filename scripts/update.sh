...
# When script is in scripts/, repo/config root is parent
...
if ! rpm -q fail2ban-server &>/dev/null; then
   echo "fail2ban is not installed. Run $INSTALL_DIR/scripts/install.sh for full installation." >&2
   exit 1
fi
[ -d "$CONFIG_DIR" ] || { echo "Source not found. Run scripts/install.sh first." >&2; exit 1; }
