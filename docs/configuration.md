...
| jail.d/*.conf | Jail definitions (backend=pyinotify, logpath, banaction). `98-domlog-backend.conf` forces pyinotify/usedns=no. apache-high-volume uses ignorecommand → cms-allow.sh |
| jail.d/apache-ua-keywords.conf | Combined User-Agent keyword jail (one jail for all keywords; generated by update-useragent-jails.sh) |
| filter.d/apache-ua-keywords.conf | Combined UA failregex (empty UA, python, headless by default). Darwin is never matched — it is in every iOS/macOS User-Agent. |
